angular.module("myApp")
    ...
    .service("$sockets",function ($rootScope,$timeout,$pop) {
        ...
        this.init=function(){
            if(!self.tried){
                self.tried=true
                socket = io.connect('https://atc-edu.com/atc');
    ...
    .service("$utils",function ($rootScope) {
        this.loadScript=function(url,id,callback) {
            let isLoaded = document.querySelectorAll('#'+id);
        ...
        this.setTitle=function (title) {
            document.getElementsByTagName("title")[0].innerHTML="African Training Center - "+title
        }
    })
    .service("$sharerTags",function () {
        var service = this;
