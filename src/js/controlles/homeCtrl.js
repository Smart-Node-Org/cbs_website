angular.module("myApp")
    .controller("homeCtrl",function ($rootScope,$scope,$mdDialog,$http2,$pop,$state,$interval,$compile) {
        console.log('hi home');
        $http2.get("getHomeIndicators")
            .then(function (resp) {
                resp.data.forEach(v=>v.value=angular.fromJson(v.value))
                $scope.homeIndicators=resp.data
                $scope.$apply()
            ...
            if(resp.data){
                ...
                $scope.Allnews1.forEach(function (t,index) {
                    ...
                    var li=" <li class=\"uk-active uk-transition-active\" >\n" +
                        ...
                        "<img src='https://smart-node.net/data/cbs/news/"+t.img+"' alt=\"\" style=\"height: 400px;width: 100%;\">\n" +
                        "</div>\n" +
                        ...
                        "<p uk-margin>\n" +
                        "\n" +
                        "<button id=\"index\" ng-click=\"oneNews("+t.id+")\" class=\"uk-button news uk-button-danger\"  style=\"border-radius: 50px;background-color: #2d3669\">ReadMore</button>\n" +
                        "\n"+
                        "</p>\n" +
                        "</div>\n" +
                        "</div>\n" +
                    ...
                    x+=li
                        ...
                        "</div>\n" +
                        "<div class=\"uk-card-body\" style=\"width: 100%;height: 260px!important; background-color: #eeeeee\">\n" +
                        "<h3 class=\"uk-card-title\" style=\"font-size:25px; font-weight: bold; color: #3e4095\">"+t.title_ar+"</h3>\n" +
                        "<p style=\"color: #888888;text-align: left\">"+t.body_ar.substr(0,100)+" ..."+"</p>\n" +
                        "<p uk-margin>\n" +
                        "\n" +
                ...
                console.log( $scope.Allnews2);
                    $scope.Allnews2.forEach(function (t,index) {
                        ...
                        var li=" <li class=\"uk-active uk-transition-active\" >\n" +
                            ...
                            "<p uk-margin>\n" +
                            "\n" +
                            "<button id=\"index\" ng-click=\"oneNews("+t.id+")\" class=\"uk-button news2 uk-button-danger\" style=\"border-radius: 50px;background-color: #2d3669\" >ReadMore</button>\n" +
                            "\n"+
                            "</p>\n" +
                            "</div>\n" +
                            "</div>\n" +
